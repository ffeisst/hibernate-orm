 ...
 * {@link org.hibernate.envers.configuration.metadata.reader.PersistentPropertiesSource} and adds the ones that are
 * audited to a {@link org.hibernate.envers.configuration.metadata.reader.AuditedPropertiesHolder}, filling all the
 * auditing data.
 * 
...
public class AuditedPropertiesReader {
	...
	public AuditedPropertiesReader(ModificationStore defaultStore, PersistentPropertiesSource persistentPropertiesSource,
			AuditedPropertiesHolder auditedPropertiesHolder, GlobalConfiguration globalCfg, ReflectionManager reflectionManager, String propertyNamePrefix) {
	 ...
	 * checks whether given class exists in {@link AuditedPropertiesReader#overriddenAuditedClasses} collection. In case
	 * of success, {@link Audited} configuration of currently mapped entity is returned, otherwise {@code null}. If
	 * processed type exists in {@link AuditedPropertiesReader#overriddenNotAuditedClasses} collection, the result is
	 * also {@code null}.
	 */
	...
	private void addPropertiesFromDynamicComponent(DynamicComponentSource dynamicComponentSource) {
		...
		if ( !fieldAccessedPersistentProperties.isEmpty() ) {
			throw new MappingException( "Audited dynamic component cannot have properties with access=\"field\" for properties: "
	...
	private void addFromProperties(Iterable<XProperty> properties, String accessType, Set<String> persistentProperties, Audited allClassAudited) {
		for ( XProperty property : properties ) {
			...
			if ( ( persistentProperties.contains( property.getName() ) && ( !auditedPropertiesHolder.contains( property.getName() ) ) ) ) {
	...
	private void addFromPropertiesGroup(String embeddedName, XProperty property, String accessType, Component propertyValue, Audited allClassAudited) {
		...
		if ( isAudited ) {
			...
			AuditedPropertiesReader audPropReader = new AuditedPropertiesReader( ModificationStore.FULL, componentPropertiesSource, componentData, globalCfg,
	...
	private void addFromComponentProperty(XProperty property, String accessType, Component propertyValue, Audited allClassAudited) {
		...
		ComponentAuditedPropertiesReader audPropReader = new ComponentAuditedPropertiesReader( ModificationStore.FULL, componentPropertiesSource,
				componentData, globalCfg, reflectionManager, propertyNamePrefix + MappingTools.createComponentPrefix( property.getName() ) );
		audPropReader.read();
	 ...
	 * 
	 * @param property the property for which the {@link org.hibernate.envers.AuditOverride} annotations are being
	 * processed
	 * @param propertyData the Envers auditing data for this property
	...
	public static class DynamicComponentSource extends ComponentPropertiesSource {
		...
		private final XProperty baseProperty;
