...
public class ComponentPropertyMapper implements PropertyMapper, CompositeMapperBuilder {
	...
	public ComponentPropertyMapper(PropertyData propertyData, String componentClassName) {
		...
		// TODO can componentClassName be the name of a subclass of Map?
		// if ( Map.class.isAssignableFrom( componentClass ) ) {
		if ( Map.class.getCanonicalName().equals( componentClassName ) ) {
			this.delegate = new MultiDynamicComponentMapper( propertyData );
			this.componentClassName = HashMap.class.getCanonicalName();
		}
		else {
	...
	public void mapModifiedFlagsToMapFromEntity(SessionImplementor session, Map<String, Object> data, Object newObj, Object oldObj) {
		if ( propertyData.isUsingModifiedFlag() ) {
			data.put( propertyData.getModifiedFlagPropertyName(), delegate.mapToMapFromEntity( session, new HashMap<String, Object>(), newObj, oldObj ) );
		}
	}
	...
	public void mapToEntityFromMap(AuditConfiguration verCfg, Object obj, Map data, Object primaryKey, AuditReaderImplementor versionsReader, Number revision) {
		...
		if ( allNullAndSingle ) {
			...
			// set the component
			try {
				Object subObj = ReflectHelper.getDefaultConstructor( ReflectHelper.classForName( componentClassName ) ).newInstance();
	...
	@Override
	public List<PersistentCollectionChangeData> mapCollectionChanges(SessionImplementor session, String referencingPropertyName, PersistentCollection newColl,
