...
/**
 * @author Lukasz Zuchowski (author at zuchos dot com) More advanced tests for dynamic component.
 */
...
public class AuditedDynamicComponentsAdvancedCasesTest extends BaseEnversFunctionalTestCase {
	...
	public void testOfQueryOnDynamicComponent() {
		...
		List resultList = getAuditReader().createQuery().forEntitiesAtRevision( AdvancedEntity.class, 1 )
				.add( AuditEntity.relatedId( "dynamicConfiguration_" + PROP_MANY_TO_ONE ).eq( manyToOneEntity.getId() ) ).getResultList();
		...
		resultList = getAuditReader().createQuery().forEntitiesAtRevision( AdvancedEntity.class, 1 )
				.add( AuditEntity.property( "dynamicConfiguration_" + INTERNAL_COMPONENT + "_property" ).eq( internalComponent.getProperty() ) )
				.getResultList();
		...
		try {
			...
			getAuditReader().createQuery().forEntitiesAtRevision( AdvancedEntity.class, 1 )
					.add( AuditEntity.property( "dynamicConfiguration_" + PROP_ONE_TO_ONE ).eq( oneToOneEntity ) ).getResultList();
		...
		try {
			getAuditReader()
					...
					.add( AuditEntity.property( "dynamicConfiguration_" + INTERNAL_MAP_WITH_MANY_TO_MANY ).eq(
							entity.getDynamicConfiguration().get( INTERNAL_MAP_WITH_MANY_TO_MANY ) ) ).getResultList();
			Assert.fail();
		}
